...
class Api
{
    ...
    public function sendOrder(Order $order)
    {
        if (empty($order->products)) {
            print 'Property "products" of class Order can\'t be empty!';
        }

        if (empty($order->customer)) {
            print 'Property "customer" of class Order can\'t be empty!';
        }
        ...
        if ($order->comment) {
            $postParams['comment'] = $order->comment;
        }

        try {
            $res = $this->client->request('post', 'https://iiko.biz:9900/api/0/orders/add?' . http_build_query($params), [
